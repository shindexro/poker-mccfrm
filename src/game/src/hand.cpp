...
HandStrength Hand::GetStrength() {
  if (cards.size() != 5) {
    throw invalid_argument(
        "Failed to determine hand strength because card count is not 5.");
  }
  ...
  int rankProduct =
      std::accumulate(cards.begin(), cards.end(), 1,
                      [](int acc, Card &c) { return acc * c.PrimeRank(); });
  int suitProduct =
      std::accumulate(cards.begin(), cards.end(), 1,
                      [](int acc, Card &c) { return acc * c.PrimeSuit(); });

  bool straight = rankProduct == 8610         // 5-high straight
  ...
  bool flush = suitProduct == 147008443     // Spades
