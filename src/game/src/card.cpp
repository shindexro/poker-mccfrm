...
bool Card::operator==(const Card &other) const { return Equals(other); }

int Card::PrimeRank() const { return rankPrimes[(int)rank]; }

int Card::PrimeSuit() const { return suitPrimes[(int)suit]; }

int Card::HashCode(Card &c) { return c.PrimeRank() * c.PrimeSuit(); }

int Card::Index() { return (int)rank * 4 + (int)suit; }

ulong Card::Bitmask() { return 1ul << Index(); }
...
string Card::ToString() const {
  ...
  char suits[] = {'s', 'h', 'd', 'c'};
  // char suits[] = {'♠', '♥', '♦', '♣'};
  string s = string({ranks[(int)rank], suits[(int)suit]});
  return s;
}
