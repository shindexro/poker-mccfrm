...
PlayState::PlayState(CommunityInfo &community, vector<PlayerInfo> &players,
...
void PlayState::GenerateUniqueStringIdentifier() {
  ...
  if (community.cards.size() == 0) {
    ...
    long index =
        EMDTable::flopIndices[Global::indexer_2_3.IndexLastRound(cards)];
    cardString += "F" + to_string(index);
  } else if (community.cards.size() == 4) {
    long index =
        EMDTable::turnIndices[Global::indexer_2_4.IndexLastRound(cards)];
    cardString += "T" + to_string(index);
  } else {
    long index =
        OCHSTable::riverIndices[Global::indexer_2_5.IndexLastRound(cards)];
...
void PlayState::CreateCallChildren() {
  // call possible if needed chips is fewer than owned stack (otherwise its all
  // in)
...
void PlayState::CreateRaiseChildren() {
  auto raiseRatios = Global::raiseRatiosByRoundByPlayerCount.at(
      community.bettingRound)[GetNumberOfPlayersThatNeedToAct()];
  ...
  for (auto i = 0UL; i < raiseRatios.size(); ++i) {
    ...
    int additionalRaise =
        raise + players[community.playerToMove].bet - MinimumCall();
    /* For example, if an opponent bets $5, a player must raise by at least
       another $5, and they may not raise by only $2. If a player raises a bet
       of $5 by $7 (for a total of $12), the next re-raise would have to be by
       at least another $7 (the previous raise) more than the $12 (for a total
       of at least $19).
    */
    ...
    static const vector<Action> raiseRatioIdxToAction{
        ...
        Action::Raise4, Action::Raise5, Action::Raise6,
        Action::Raise7, Action::Raise8, Action::Raise9,
    };
...
void PlayState::CreateAllInChildren() {
  // (currently, multiple all-ins in a row dont accumulate the raises and
  // re-open betting round but probably they should)
  ...
  if (nextState->community.playerToMove != -1) {
    ...
    ++nextState->community.bettingRound;
    children.push_back(make_shared<ChanceState>(
        nextState->community, nextState->players, nextState->history));
  } else {
    children.push_back(make_shared<TerminalState>(
        nextState->community, nextState->players, nextState->history));
  }
}
