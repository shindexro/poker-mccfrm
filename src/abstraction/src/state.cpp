...
namespace poker {
State::State()
    : community(), players(Global::nofPlayers, PlayerInfo()), history() {}

State::State(CommunityInfo &community, vector<PlayerInfo> &players,
             vector<Action> &history)
    : community(community), players(players), history(history), children(),
      infosetString() {}
...
int State::PrevActivePlayer() {
  int last = -1;
  for (auto i = (community.playerToMove + 1) % Global::nofPlayers;
       i != community.playerToMove; i = (i + 1) % Global::nofPlayers) {
    if (players[i].IsAlive() && players[i].lastAction != Action::Allin) {
      last = i;
    }
  }
...
int State::GetNumberOfActivePlayers() {
  return count_if(players.begin(), players.end(),
                  [](PlayerInfo &p) { return p.IsAlive(); });
}

int State::GetNumberOfAllInPlayers() {
  return count_if(players.begin(), players.end(),
                  [](PlayerInfo &p) { return p.lastAction == Action::Allin; });
}
...
int State::BettingRound() { return community.bettingRound; }
