...
namespace poker {
TerminalState::TerminalState() : State() { evaluator = Global::handEvaluator; }

TerminalState::TerminalState(CommunityInfo &community,
                             vector<PlayerInfo> &players,
                             vector<Action> &history)
    : State(community, players, history), rewardGenerated{false} {
...
void TerminalState::CreateRewards() {
  ...
  rake = min(rake, Global::SB); // assume rake cap at 1SB
  pot -= rake;
  ...
  if (GetNumberOfActivePlayers() == 1) {
    ...
    for (auto i = 0; i < Global::nofPlayers; ++i) {
      ...
      ulong cardsBitmask =
          players[i].GetCardBitmask() | community.GetCardBitmask();
    ...
    for (auto i = 0UL; i < playersWithBestHand.size(); ++i) {
      players[playersWithBestHand[i]].reward +=
          pot / playersWithBestHand.size();
    }
  }
