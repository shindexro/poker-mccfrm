...
typedef phmap::parallel_flat_hash_map<
    string, Infoset, phmap::priv::hash_default_hash<string>,
    phmap::priv::hash_default_eq<string>,
    std::allocator<std::pair<const string, Infoset>>, 12, phmap::AbslMutex>
    NodeMap;
...
public:
  ...
  ;
  static const map<BettingRound, vector<vector<float>>>
      raiseRatiosByRoundByPlayerCount;
  ...
  // 169 - 200 - 200 - 200 abstraction, where the river is bucketed using OCHS
  // and the turn and flop using EMD
  ...
  // 100k or even 1 million shouldn't take too much time compared to the rest of
  // the information abstraction
  inline static const int nofMCSimsPerPreflopHand = 1000000;
  // for the river, determines the river histogram size (in theory could be up
  // to 169 but will be very slow) default 8
  ...
  // this is used to create the nofOpponentClusters, it can be increased
  // (default 50) with little time penalty because the clustering for 169 hands
  // is very fast
