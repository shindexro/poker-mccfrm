...
void EMDTable::GenerateTurnHistograms() {
  std::cout << "Generating histograms for " << Global::indexer_2_4.roundSize[1]
            << " turn hands of length " << Global::nofRiverBuckets << " each..."
            << endl;
  chrono::steady_clock::time_point start = chrono::steady_clock::now();

  histogramsTurn = vector<vector<float>>(
      Global::indexer_2_4.roundSize[1], vector<float>(Global::nofRiverBuckets));
  ...
  std::cout << "Time taken to generate turn histograms: " << elapsed << "[s]"
            << endl;
}

void EMDTable::GenerateFlopHistograms() {
  std::cout << "Generating histograms for " << Global::indexer_2_3.roundSize[1]
            << " flop hands of length " << Global::nofTurnBuckets << " each..."
            << endl;
  chrono::steady_clock::time_point start = chrono::steady_clock::now();

  histogramsFlop = vector<vector<float>>(Global::indexer_2_3.roundSize[1],
                                         vector<float>(Global::nofTurnBuckets));
  ...
  auto elapsed =
      chrono::duration_cast<std::chrono::seconds>(end - start).count();
  cout << "Time taken to generate flop histograms: " << elapsed << "[s]"
       << endl;
}
...
void EMDTable::ClusterTurn() {
  ...
  turnIndices =
      kmeans.ClusterEMD(histogramsTurn, Global::nofTurnBuckets, 1, indices);
  ...
  std::cout << "Created the following clusters for the Turn (extract of one "
               "cluster): "
            << endl;
  int nofEntriesToDisplay = 20;
  for (auto i = 0L;
       i < Global::indexer_2_4.roundSize[1] && nofEntriesToDisplay > 0; ++i) {
    if (turnIndices[i] == 0) {
      auto cards = vector<int>(6);
      ...
      Hand hand = Hand();
      hand.cards =
          vector<Card>({Card(cards[0]), Card(cards[1]), Card(cards[2]),
                        Card(cards[3]), Card(cards[4]), Card(cards[5])});
...
void EMDTable::ClusterFlop() {
  ...
  auto indices = vector<int>();
  flopIndices =
      kmeans.ClusterEMD(histogramsFlop, Global::nofFlopBuckets, 1, indices);
  ...
  cout << "Created the following clusters for the Flop (extract of one "
          "cluster): "
       << endl;
  int nofEntriesToDisplay = 20;
  for (auto i = 0L;
       i < Global::indexer_2_3.roundSize[1] && nofEntriesToDisplay > 0; ++i) {
    if (turnIndices[i] == 0) {
      auto cards = vector<int>(5);
      Global::indexer_2_3.Unindex(Global::indexer_2_3.rounds - 1, i, cards);

      Hand hand = Hand();
      hand.cards = vector<Card>({Card(cards[0]), Card(cards[1]), Card(cards[2]),
                                 Card(cards[3]), Card(cards[4])});
      hand.PrintColoredCards("\n");
      nofEntriesToDisplay--;
    }
  }
